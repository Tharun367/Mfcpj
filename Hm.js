...
let bloodgrp = document.querySelector('#bloodgrp');
...
keyPath: 'key', autoIncrement: true } );
 ...
 objectStore.createIndex('bloodgrp', 'bloodgrp', { unique: false } );
...
DB.transaction('consultations').objectStore('consultations');
 ...
 ConsultationHTML.setAttribute('data-consultation-id', 
cursor.value.key);
 ConsultationHTML.classList.add('list-group-item');
 
 
 ConsultationHTML.innerHTML = `
 <p class="font-weight-bold">Donor Name: <span 
class="font-weight-normal">${cursor.value.patientname}<span></p>
 <p class="font-weight-bold">Mobile No: <span 
class="font-weight-normal">${cursor.value.contact}<span></p>
 <p class="font-weight-bold">DOB: <span 
class="font-weight-normal">${cursor.value.date}<span></p>
 <p class="font-weight-bold">Blood Group: <span 
class="font-weight-normal">${cursor.value.bloodgrp}<span></p>
 `;
 ...
 services.textContent = 'DONOR DETAILS';
